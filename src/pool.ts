...
interface ConnectionPoolEvents {
  ...
  started: () => unknown,
  stopped: () => unknown,
  created: (connection: Connection) => unknown,
  destroyed: (connection: Connection) => unknown,
  acquired: (connection: Connection) => unknown,
  released: (connection: Connection) => unknown,
}
...
export class ConnectionPool extends Emitter<ConnectionPoolEvents> {
  ...
  private _borrowed = new Map<Connection, NodeJS.Timeout>()
  // Array of all _available_ connections
  private _available: Connection[] = []
  // Array of all pending connection requests
  private _pending: ConnectionRequest[] = []

  private _connections = new WeakSet<Connection>()

  private _minimumPoolSize
  private _maximumPoolSize
  private _maximumIdleClients
  private _acquireTimeoutMs
  private _borrowTimeoutMs
  private _createRetryIntervalMs

  private _connectionOptions: string
  ...
  constructor(name: string, logger: Logger, options: ConnectionPoolOptions) {
    ...
    this._maximumIdleClients = Math.floor(maximumIdleClients)
    ...
    assert(this._maximumIdleClients <= this._maximumPoolSize,
        `he maximum number of idle clients ${this._maximumIdleClients} must less or equal to the maximum pool size ${this._maximumPoolSize}`)

    logger.warn({
      minimumPoolSize: this._minimumPoolSize,
      maximumPoolSize: this._maximumPoolSize,
      maximumIdleClients: this._maximumIdleClients,
    })
  ...
  get stats(): { available: number, borrowed: number, total: number } {
    const available = this._available.length
    const borrowed = this._borrowed.size
    const total = available + borrowed
    return { available, borrowed, total }
  }
  ...
  protected _destroy(connection: Connection): void {
    ...
    /* If asked to disconnect, force disconnection */
  ...
  private _runCreateLoop(): void {
    ...
    process.nextTick(() => {
      ...
      const connections = this._available.length + this._borrowed.size
      if ((this._available.length && (connections >= this._minimumPoolSize)) ||
          (connections >= this._maximumPoolSize)) {
        return
      }

      this._logger.info('Creating new pooled connection for pool', this.stats)
      ...
      /* Dispose of the connection when the pool is destroyed */
      const evictor = (): void => connection.disconnect()
      this.once('destroyed', evictor)

      /* Dispose of the connection when the connection is closed */
      connection.once('disconnected', () => {
        this.off('destroyed', evictor)
      ...
      connection.connect().then(() => {
        /* The pool might have been destroyed while connecting... */
  ...
  private _runBorrowLoop(): void {
    ...
    process.nextTick(() => Promise.resolve().then(async () =>{
      ...
      while (request = this._pending.splice(0, 1)[0]) {
        ...
        /* The pool might have been destroyed while validating, simply return
         * and let the "destroyed" event handler do its job */
        if (! this._started) return
        ...
        if (! valid) {
          ...
          connection.disconnect() // will trigger the "disconnected" event
          continue
        }
  ...
  release(connection: Connection): void {
    Promise.resolve().then(async () => {
      ...
      const timeout = this._borrowed.get(connection)
      this._borrowed.delete(connection)
      ...
      if (! connection.connected) {
        ...
        connection.disconnect() // might trigger the "disconnect" event, better safe than sorry
      ...
      /* If we have enough available connections, discard it */
        ...
        connection.disconnect() // definitely will trigger the "disconnect" event
      ...
      /* If the connection is not valid, discard it */
        ...
        connection.disconnect() // might trigger "disconnect" in case of SQL errors
    ...
    /* Any error might come from trying to validate/recycle the connection */
      ...
      connection.disconnect() // once again, might trigger "disconnect" in case of SQL errors
  ...
  stop(): void {
    ...
    this._logger.info('Destroying connection pool')
    ...
    /* Let the "destroy" event handler close up all connections */
